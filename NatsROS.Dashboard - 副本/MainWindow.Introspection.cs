    ...
    public partial class MainWindow
    {
        private async void BtnGenerateGraph_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                var replies = _nats.Connection.RequestManyAsync<NodeIntrospectionReq, NodeIntrospectionRes>("natsros.introspection.ping", new NodeIntrospectionReq(Guid.NewGuid().ToString()), cancellationToken: cts.Token);
                ...
                await foreach (var reply in replies)
                {
                    ...
                    if (!drawnNodes.TryGetValue(info.NodeName, out var nodeShape))
                    {
                        ...
                        drawnNodes[info.NodeName] = nodeShape; DiagramGraph.Items.Add(nodeShape);
                    }
        ...
        private void ApplyAutoLayout() { if (DiagramGraph.Items.Count > 0) { DiagramGraph.ApplySugiyamaLayout(Direction.Right); DiagramGraph.FitToDrawing(); } }
